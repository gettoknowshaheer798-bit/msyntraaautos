...
import { vehicles } from "@/data/vehicles"; // Adjust path if needed
import {
    ArrowRight,
    Car,
    ChevronDown,
...
// Helper to parse price string like "$63,700" to number 63700
...
export default function FinancingCalculatorSection() {
  ...
  const [selectedVehicleId, setSelectedVehicleId] = useState<string>(
    vehicles[4]?.id || vehicles[0].id
  );

  // Active Vehicle Data for Calculator & Bottom Banner
  ...
  // Calculator State initialized dynamically
  ...
  const handleVehicleChange = (vehicleId: string) => {
    ...
    if (veh) {
      const parsedPrice = parsePrice(veh.price);
      setSelectedVehicleId(vehicleId);
      setVehiclePrice(parsedPrice);
      setDownPayment(Math.min(downPayment, parsedPrice));
    }
  };
  ...
  return (
    <div className="bg-[#f4f0eb] text-[#0d1c17] min-h-screen font-sans pt-24 w-full">
      ...
      {/* ------------------------------------------------------------- */}
      {/* HERO SECTION (STATIC FULL PHOTO WITH SEAMLESS FADE MASK)       */}
      {/* ------------------------------------------------------------- */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center w-full">
          ...
          <div className="lg:col-span-5 z-10 pl-8 md:pl-16 lg:pl-24 py-12 md:py-20">
            ...
            <p className="text-xs md:text-sm text-[#5d6863] font-light leading-relaxed mb-8 max-w-sm">
              Flexible financing options designed around you. Competitive rates, transparent terms, and a seamless process from start to finish.
            </p>
            <a
              href="#calculator"
            ...
            >
              CALCULATE YOUR PAYMENT <ArrowRight className="w-3.5 h-3.5" />
            </a>
          </div>

          {/* Right Image Container with Gradient Fade Mask */}
      ...
      {/* ------------------------------------------------------------- */}
      <section id="calculator" className="w-full px-8 md:px-16 lg:px-24 py-20">
        <div className="w-full bg-[#eae5dd] p-6 md:p-12 border-t border-b border-[#dcd5c9]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
            ...
            <div className="lg:col-span-7 space-y-8 flex flex-col justify-between pr-0 lg:pr-6">
              ...
              <div>
                <div className="flex justify-between items-baseline mb-2">
                  <label htmlFor={priceInputId} className="text-[10px] tracking-[0.2em] uppercase font-semibold text-[#5d6863]">
                    VEHICLE PRICE
                  </label>
                </div>
                ...
                <div className="flex justify-between text-[10px] text-[#8a9992] font-mono mt-2">
                  <span>$10,000</span>
                  <span>${Math.max(4000000, currentVehicleNumericPrice * 1.2).toLocaleString()}</span>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-baseline mb-2">
                  <label htmlFor={downPaymentInputId} className="text-[10px] tracking-[0.2em] uppercase font-semibold text-[#5d6863]">
                    DOWN PAYMENT
                  </label>
                </div>
                <div className="text-3xl font-serif font-light text-[#0d1c17] mb-3">
              ...
              <div>
                <div className="flex justify-between items-baseline mb-2">
                  <label htmlFor={termInputId} className="text-[10px] tracking-[0.2em] uppercase font-semibold text-[#5d6863]">
                    TERM
                  </label>
                </div>
                <div className="text-3xl font-serif font-light text-[#0d1c17] mb-3">
              ...
              <div>
                <div className="flex justify-between items-baseline mb-2">
                  <label htmlFor={aprInputId} className="text-[10px] tracking-[0.2em] uppercase font-semibold text-[#5d6863]">
                    APR
                  </label>
                </div>
                <div className="text-3xl font-serif font-light text-[#0d1c17] mb-3">
            ...
            <div className="lg:col-span-5 bg-[#0d1c17] text-[#e7e3dc] p-8 md:p-10 flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-4 text-xs font-light">
                  <div className="flex justify-between text-[#8a9992]">
                    ...
                    <span className="text-white font-medium">{currentVehicle.make} {currentVehicle.model}</span>
                  </div>
                  <div className="flex justify-between text-[#8a9992]">
                    <span>Vehicle Price</span>
                    <span className="text-white font-mono">${vehiclePrice.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-[#8a9992]">
                  ...
                  <div className="flex justify-between text-[#8a9992]">
                    ...
                    <span className="text-white font-mono">${amountFinanced.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-[#8a9992]">
                    <span>APR</span>
                    <span className="text-white font-mono">{apr.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between text-[#8a9992]">
                    <span>Term</span>
                    <span className="text-white font-mono">{term} Months</span>
                  </div>
                </div>
              </div>
              ...
              >
                APPLY FOR THIS VEHICLE <Sliders className="w-3.5 h-3.5" />
              </Link>
            </div>
          </div>
          ...
          <p className="flex items-center gap-1.5 text-[10px] text-[#8a9992] font-light mt-6">
            ...
            This is an estimate only and not a guarantee of credit. All financing is subject to credit approval.
          </p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-0 lg:divide-x lg:divide-[#dcd5c9] w-full">
          ...
          <div className="lg:pr-8 flex flex-col items-start text-left">
            ...
            <p className="text-xs text-[#5d6863] font-light leading-relaxed max-w-xs">
              Browse our collection and find the vehicle that’s right for you.
            </p>
          </div>
          ...
          <div className="lg:px-8 flex flex-col items-start text-left">
            ...
            <p className="text-xs text-[#5d6863] font-light leading-relaxed max-w-xs">
              Use our calculator to adjust payment options that fit your budget.
            </p>
          </div>
          ...
          <div className="lg:px-8 flex flex-col items-start text-left">
            ...
            <p className="text-xs text-[#5d6863] font-light leading-relaxed max-w-xs">
              Secure application process with quick decisions and real support.
            </p>
          </div>
      ...
      {/* ------------------------------------------------------------- */}
      {/* START YOUR APPLICATION BOTTOM BANNER                          */}
      {/* ------------------------------------------------------------- */}
      <section className="w-full bg-[#0d1c17] grid grid-cols-1 lg:grid-cols-12 min-h-[260px]">
        ...
        <div className="lg:col-span-7 p-8 md:p-16 flex flex-col justify-center items-start text-[#e7e3dc]">
          ...
          <h2 className="font-serif text-2xl md:text-4xl font-light uppercase tracking-wider mb-4 flex items-center gap-3">
            START YOUR APPLICATION <ArrowRight className="w-6 h-6 text-[#9e6d48]" />
          </h2>
          <p className="text-xs text-[#8a9992] font-light max-w-md mb-6">
            It only takes a few minutes and could get you one step closer to your new {currentVehicle.make} {currentVehicle.model}.
          </p>
          <Link
            href="/contact"
            className="px-8 py-3.5 bg-[#9e6d48] text-[#0d1c17] text-[10px] tracking-[0.25em] uppercase font-semibold hover:bg-white transition-all inline-flex items-center gap-2"
          >
            START YOUR APPLICATION <FileCheck className="w-3.5 h-3.5" />
          </Link>
        </div>
      </section>
    </div>
